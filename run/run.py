...
class run():
    ...
    def load_cvdata(self):
        # Reuse the same split files as cross-validation, but only for model replay/evaluation.
    ...
    def load_fea(self):
        # Load feature maps once and index them later by drug/protein IDs.
    ...
    def inits(self, fold, cv_data, id2idx, fea_drug, fea_prot):
        # Slice a single fold into aligned drug/protein tensors for standalone evaluation.
        ...
        if self.params.source == 'bindingdb':
            ...
            train.to_csv(prj_path / 'data' / 'processed_data' / 'split_cvdata' / f'{self.params.source}' / 'train_run_k.csv')
            valid.to_csv(prj_path / 'data' / 'processed_data' / 'split_cvdata' / f'{self.params.source}' / 'valid_run_k.csv')
            test.to_csv(prj_path / 'data' / 'processed_data' / 'split_cvdata' / f'{self.params.source}' / 'test_run_k.csv')
        else:
            (train, valid, test) = cv_data[fold]
            print(f'source: {self.params.source}')
            train = pd.concat([train.loc[train['source']==src] for src in self.params.source.split(",")]).sort_index()
            valid = pd.concat([valid.loc[valid['source']==src] for src in self.params.source.split(",")]).sort_index()
            test = pd.concat([test.loc[test['source']==src] for src in self.params.source.split(",")]).sort_index()
            train.to_csv(prj_path / 'data' / 'processed_data' / 'split_cvdata' /  f'{self.params.source}' / f'{fold}th_fold' / 'train_run_k.csv')
            valid.to_csv(prj_path / 'data' / 'processed_data' / 'split_cvdata' /  f'{self.params.source}' / f'{fold}th_fold' / 'valid_run_k.csv')
            test.to_csv(prj_path / 'data' / 'processed_data' / 'split_cvdata' /  f'{self.params.source}' / f'{fold}th_fold' / 'test_run_k.csv')
    ...
    def run(self,):
        # Load saved checkpoints and regenerate train/valid/test metrics without retraining.
        ...
        for fold in cv_data.keys():
            ...
            # fit finished
            trainY_pred, _latents_d, _latents_p = clf.run_loop(X=trainX,batch_size=self.params.batch_size)
            ...
            validY_pred, _latents_d, _latents_p = clf.run_loop(X=validX,batch_size=self.params.batch_size)
            ...
            testY_pred, _latents_d, _latents_p = clf.run_loop(X=testX,batch_size=self.params.batch_size)
    ...
    def save(self, assess, allfold_train_data, allfold_val_data, allfold_test_data):
        # Save one row per fold because this mode keeps only the final replay result.
