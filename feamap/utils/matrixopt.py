...
class Scatter2Grid:
    ...
    def fit(self, df, split_channels = True, channel_col = [], channel_order = []):
        """
        parameters
        ------------------
        ...
        """
        # Store the original feature row index before solving the assignment problem.
        df['idx'] = range(len(df))
        ...
        grid_map = grid[:N]
        # Assign each embedded feature to one grid slot with minimum global distance.
        ...
        if self.split_channels:
            ...
            # sidx = df.groupby(channel_col).apply(_apply_split) ### pd.DataFrame({groupA:{idx:[x1,x2,...], indices:[y1,y2,...]}, groupB:{idx:[x11,x22,...], indices:[y11, y22, ...]}, ...}).T
            if not channel_col==[]:
                # sidx = pd.DataFrame.from_dict({channel: df.loc[df[channel] == 1]['idx', 'indices'].to_dict('list') for channel in channel_col}).T
                sidx = pd.DataFrame.from_dict({channel: _apply_split(df.loc[df[channel] == 1]) for channel in channel_col}).T
                ...
                # indices_list = sidx.values.tolist()
                indices_list = sidx.to_dict('records')
    ...
    def transform(self, vector_1d):
        ...
        """
        # Scatter one flat feature vector back to the grid positions solved during fit.
...
class Scatter2Array:
    ...
    def _fit(self, df):
        """df: dataframe with x, y columns"""
        # Precompute the regular lattice used to bin continuous coordinates.
        M, N = self.fmap_shape
    ...
    def _transform(self, dfnew):
        ...
        """
        # Map each point to the nearest array cell in the predefined lattice.
        x = dfnew.x.values
        y = dfnew.y.values
        M, N = self.fmap_shape
        indices = []
    ...
    def fit(self, df, split_channels = True, channel_col = [], channel_order = []):
        ...
        if self.split_channels:
            ...
            if not channel_col==[]:
                sidx = pd.Series({channel: self._transform(df.loc[df[channel] == 1]) for channel in channel_col}).T
                if len(channel_order) != 0:
                    sidx = sidx.loc[channel_order]            
    ...
    def transform(self, vector_1d):
        ...
        # Fill the grid by writing feature values into their assigned flat indices.
        M, N = self.fmap_shape
        arr = np.zeros(self.fmap_shape)
        arr_1d = arr.reshape(M*N, )
