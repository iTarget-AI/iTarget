...
"""
from feamap.utils.distances import named_distances
from feamap.utils.multiproc import MultiProcessUnorderedBarRun
...
def _yield_combinations(N):
    # Only emit the lower triangle; the distance matrix is symmetric.
    for i1 in range(N):
        for i2 in range(i1):
            yield (i1,i2)
            
def _calculate(i1, i2):
    # Distances are computed feature-wise after dropping rows with missing values.
    x1 = data[:, i1]
    x2 = data[:, i2]
    ## dropna
...
def pairwise_distance(npydata, n_cpus=8, method='correlation'):
    ...
    >>> (10,10)  
    """    
    # The multiprocessing runner reads globals, so expose both data and metric function here.
    global data, func
    ...
    # Mirror the lower-triangle results back into a full square matrix.
