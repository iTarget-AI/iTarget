...
class Base:
    ...
    def MinMaxScaleClip(self, x, xmin, xmax):
        # Match the scaling rule used when the template statistics were prepared.
...
class Map(Base):
    ...
    def __init__(self, disttype = 'uniprot+fullchembl', datatype = 'protein_fea', ftype = 'esm2', flist = [], fmap_type = 'grid', fmap_shape = None, split_channels = True, metric = 'cosine', var_thr = 1e-4, ):
        ...
        # Load the precomputed distance matrix and scaling summary for the requested feature space.
        ...
        # Keep a deterministic feature order so all generated maps share the same layout.
    ...
    def _fit_embedding(self, 
        ...
        """
        # Reduce the feature distance matrix into a 2D layout before the grid assignment step.
    ...
    def transform(self, arr, scale = True, scale_method = 'minmax',):
        ...
        # Filter and scale one feature vector, then place it onto the fitted 2D layout.
        if scale:
    ...
    def batch_transform(self, 
        ...
        # Transform one sample at a time to preserve index order in the exported arrays.
    ...
    def rearrangement(self, orignal_X, target_mp):
        ...
        """
        # Reuse feature values while swapping only the spatial arrangement strategy.
