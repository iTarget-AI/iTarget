...
def pair_statistic(label, type):
    # Summarize how many positive and negative pairs each drug/protein participates in.
...
def pair_DOWNsampling(params, label):
    # Balance positives and negatives within each drug group before model splitting.
    _label = []
...
def data_washing(params, label):
    ...
    # Reserve a stratified holdout set after optional washing and down-sampling.
...
def drug_data(data_source):
    # Prefix IDs with the source name so multi-source datasets remain collision-free.
...
def protein_data(data_source):
    # Prefix IDs with the source name so protein identifiers stay unique across datasets.
    if ',' not in data_source:
...
def relationship_data(data_source):
    # Load the interaction index table prepared by preprocessing.
    if ',' not in data_source:
...
def split_data(params, label, ):
    if ',' not in params.source:
        ...
        for s in sources:
            ...
            # Stratify within each source first, then merge the folds back together.
...
def drugllmfea_from_local(drug, feature_dict=None, type='xmol'):
    # Recover the drug feature subset and its channel metadata from local CSV exports.
...
def protllmfea_from_local(protein, feature_dict=None, type='esm2'):
    # Recover the protein feature subset and its channel metadata from local CSV exports.
...
def to_dist_matrix(data, datatype, idx, tag, methods = ['correlation', 'cosine', 'jaccard']):
    # Cache multiple distance views because different map layouts may use different metrics.
    df_dic = {}
...
def fea_statistic(fea):
    # Compute per-dimension summary statistics used later for feature normalization.
