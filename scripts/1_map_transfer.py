...
def fit_map(disttype = 'ALL', datatype = 'protein_fea', ftype = 'esm2', flist=None, split_channels=True, metric = 'cosine', fitmethod = 'umap'):
    # Fit a 2D feature layout once and serialize it as the reusable template map.
...
def load_map(disttype = 'ALL', datatype = 'protein_fea', ftype = 'esm2', flist=None, split_channels=True, metric = 'cosine', fitmethod = 'umap'):
    # This helper keeps the same plotting side effects as fit_map when a cached map is reused.
    # mp = basemap.Map(disttype = disttype, datatype = datatype, ftype = ftype, flist = flist, fmap_type='grid', split_channels=split_channels,  metric=metric, var_thr=0)
    mp = basemap.Map(disttype = disttype, datatype = datatype, ftype = ftype, flist = flist, fmap_type='grid', split_channels=split_channels,  metric=metric, var_thr=1e-4)
...
def trans_map(params, data_df, mp, datatype, ftype):
    # Convert tabular LM features into image-like maps aligned with the fitted template.
...
if __name__=='__main__':
    ...
    parser.add_argument("--disttype", type=str, default='uniprot+fullchembl', choices=['ALL','chembl','bindingdb','biosnap','celegans','human','uniprot+fullchembl'], help="the data source that the task working on")
    ...
    # The shell scripts pass booleans as strings, so normalize them here once.
    ...
    if params.datatype == 'drug_fea':
        # Fit a drug map and project the requested dataset onto that layout.
        ...
        # Fit a protein map and project the requested dataset onto that layout.
