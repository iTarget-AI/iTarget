...
def MinMaxScaleClip(x, xmin, xmax):
    # Reuse the stored feature-wise min/max statistics before distance calculation.
    # print("MinMaxScaleClip")
    scaled = (x - xmin) / ((xmax - xmin) + 1e-8)
    return scaled.clip(0, 1)

def StandardScaler(x, xmean, xstd):
    # Keep the scaling logic consistent with the configuration cached for each feature set.
...
if __name__=='__main__':
    ...
    # Save the normalization config and the derived distance matrices side by side.
    ...
    # Summarize each feature dimension so later runs can reuse a stable scaling rule.
    ...
    # Persist normalized features first, then convert them into pairwise distance matrices.
