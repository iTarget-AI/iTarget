...
from run.inference import predict
import argparse
import torch as th
import numpy as np
import random
...
def seed_torch(seed=1029):
	# Make feature generation, training, and inference runs reproducible across backends.
	random.seed(seed)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--disttype", type=str, default='uniprot+fullchembl', choices=['uniprot+fullchembl','bindingdb'], help="the data source that the task working on")
    parser.add_argument("--uncertainty_iteration_size", type=int, default=1)
    parser.add_argument("--pretrained_model_path", type=str, default='pretrained/iTarget_final_model')
    ...
    # Route the same CLI entrypoint to training, cross-validation, or inference.
    if params.task=='cv':
