...
def load_model(params, model_path, in_channels, gpuid=-1):
    # Rebuild the network with the same hyperparameters and load the stored weights.
...
def save_model(model, model_path):
    # Save on CPU to avoid device-specific loading issues on a different machine.
    model.to("cpu")
...
class MultimapCNN_dataset(Dataset):
    def __init__(self, X, Y):
        super().__init__()
        # Pair each drug map with its protein map so the dataloader returns aligned samples.
        count = len(Y)
        self.src,  self.trg = [], []
...
class EarlyStopping:
    ...
    def __call__(self, score, model, model_path):
        # Interpret lower loss as better, while all other monitored metrics are maximized.
...
class MultimapCNN(nn.Module):
    def __init__(self, params, in_channels=None):
        ...
        # Drug and protein branches use separate convolution blocks before fusion.
    ...
    def forward(self, drug_inputs, prot_inputs):
        # Encode the drug map and protein map independently, then concatenate both embeddings.
        ## first inputs
    ...
    def get_logits(self, drug_inputs, prot_inputs):
        # Keep feature extraction and classification split so embeddings can be reused elsewhere.
    ...
    def ContrastiveLoss(self, sum, label):
        # Encourage samples with the same label to stay closer in the latent space.
        num = len(sum)
        ...
        contras_label = (label1 != label2).int()
    ...
    def train_loop(self, dataloader, optimizer):
        self.to(self.device)
        loss_fn = nn.CrossEntropyLoss(weight=th.from_numpy(np.array([1,3])).float().to(self.device)) # have softmax layer
        ...
        for batch, (X, y) in enumerate(dataloader):
            ...
            # Optimize both label supervision and latent-space separation in one step.
    ...
    def run_loop(self, X, batch_size = 1, uncertainty_flag = False):
        ...
        size = len(dataloader.dataset)
        num_batches = len(dataloader)
        # Dropout stays active during uncertainty estimation so repeated runs can vary.
        self.eval() if not uncertainty_flag else self.train() # for uncertainty
        run_logits = []
        latents_d = []
        latents_p = []
        with th.no_grad():
